function rgb_out = create_1x2_block(blocks, colors, size)
...
for i=1:length(blocks)
    ...
    temp(1:2,size-3:size+3, :) = brighten(temp(1:2,size-3:size+3, :), bright);
    temp(size-1:size, size-3:size+3, :) = brighten(temp(size-1:size, size-3:size+3, :), dark);
